...
const MODES = ['research-loop', 'memory-evidence', 'fleet-benchmark', 'plugin-release', 'incident-review'];
const SKILL_FILES = {
...
function packageRoot(packageName) {
    ...
    for (const moduleRoot of moduleRoots) {
        try {
            resolvers.push(createRequire(join(dirname(moduleRoot), 'package.json')));
        }
        catch { /* ignore invalid search roots */ }
    }
...
async function inspectRuntimePackage(packageName, marker) {
    const root = packageRoot(packageName);
    if (!root)
        return { package: packageName, present: false };
...
export function apply(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (!pkgRaw)
                return asJson({ ok: false, repository, error: 'no package.json at repository root' });
            let pkg;
            try {
            ...
            add('dsh.bundle declared', 'blocker', Boolean(bundle), bundle ? 'present' : '缺失：只声明 dsh.client 无法通过 dsh plugin add 安装');
            ...
            add('cordis patch file exists', 'blocker', Boolean(patchRaw), patchRaw ? patchRef : `${patchRef} 不存在，profile 无法插入这个插件`);
            ...
            const patchNames = [...patchRaw.matchAll(/^\s*name:\s*(\S+)/gm)]
                ...
                .filter(Boolean);
            add('patch row matches package name', 'blocker', !patchRaw || patchNames.includes(String(pkg.name)), patchRaw ? `patch names=[${patchNames.join(', ')}] package.json name=${String(pkg.name)}` : 'skipped (no patch file)');
            // Publishability.
            add('publishable to npm', 'warning', pkg.private !== true, pkg.private === true ? '"private": true 会让 npm publish 直接失败' : 'not marked private');
            add('files[] declared', 'warning', Array.isArray(pkg.files) && pkg.files.length > 0, Array.isArray(pkg.files) ? `${pkg.files.length} entries` : '未声明 files[]，发布内容不可控');
            add('exports declared', 'warning', Boolean(pkg.exports), pkg.exports ? 'present' : '未声明 exports');
            ...
            add('harness packages kept as peers', 'warning', misplaced.length === 0, misplaced.length > 0 ? `应放进 peerDependencies：${misplaced.join(', ')}` : 'none in dependencies');
            ...
            const silentlySkipping = [];
            for (const dir of ['test', 'tests']) {
                const entries = await readdir(join(repository, dir)).catch(() => []);
                for (const entry of entries.slice(0, 40)) {
                    if (!entry.endsWith('.mjs'))
                        continue;
                    const source = await readFile(join(repository, dir, entry), 'utf8').catch(() => '');
                    if (!source.includes('SKIPPED') || !/process\.exit\(0\)/.test(source))
                        continue;
                    if (/process\.env\.[A-Z_]*STRICT/.test(source))
                        continue;
                    silentlySkipping.push(relative(repository, join(repository, dir, entry)));
                }
            }
            add('boot test cannot silently pass', 'blocker', silentlySkipping.length === 0, silentlySkipping.length > 0
